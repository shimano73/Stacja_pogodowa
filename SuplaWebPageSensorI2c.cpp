...
#ifdef SUPLA_BME280
    addFormHeader(webContentBuffer, String("Ustawienia BME280") );
...
#ifdef SUPLA_BMP280
    addFormHeader(webContentBuffer, String("Ustawienia BMP280") );
...
#ifdef SUPLA_SHT3x
    addFormHeader(webContentBuffer, String("Ustawienia HECA (sht30x)"));
...
#ifdef SUPLA_BH1750
    addFormHeader(webContentBuffer, String("Ustawienia czujnika nasłonecznienia"));
    selected = ConfigManager->get(KEY_ACTIVE_SENSOR)->getElement(SENSOR_I2C_BH1750).toInt();
    addListBox(webContentBuffer, INPUT_BH1750, S_BH1750, SHT3x_P, 4, selected);
    addFormHeaderEnd(webContentBuffer);
#endif
